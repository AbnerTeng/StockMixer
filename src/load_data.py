    ...
    for index, ticker in enumerate(tqdm(tickers)):
        ...
        for row in range(single_EOD.shape[0]):
            if abs(single_EOD[row][-1] + 1234) < 1e-8:
                ...
                ground_truth[index][row] = (single_EOD[row][-1] - single_EOD[row - steps][-1]) / single_EOD[
                    row - steps
                ][-1]
...
def build_SFM_data(data_path, market_name, tickers):
    ...
    for index, ticker in enumerate(tickers):
        ...
        if index == 0:
            print("single EOD data shape:", single_EOD.shape)
            eod_data = np.zeros([len(tickers), single_EOD.shape[0]], dtype=np.float32)
        ...
        for row in range(single_EOD.shape[0]):
            if abs(single_EOD[row][-1] + 1234) < 1e-8:
                if row < 3:
                    ...
                    eod_data[index][row] = np.sum(eod_data[index, row - 3 : row]) / 3
            else:
